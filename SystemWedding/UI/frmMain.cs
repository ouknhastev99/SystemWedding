...
namespace SystemWedding.UI
{
    public partial class frmMain : Form
    {
        ...
        public void LoadMoney()
        {
            ...
            if (login._ErrorCode == 0)
            {
                string str = "select sum(KH)as Khmer,sum(USD) as USD from tbProject where projectType in('" + ProjectType + "')and projectcode in('" + ProjectName + "')";
                login._ad = new System.Data.SqlClient.SqlDataAdapter(str, login._con);
                login._ad.Fill(dt);
                foreach (DataRow row in dt.Rows)
                {
                    txtRiel.Text = row[0].ToString();
                    txtUSD.Text = row[1].ToString();
                }
            }
            else
            {
